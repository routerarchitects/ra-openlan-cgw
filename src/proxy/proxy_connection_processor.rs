...
impl ProxyConnectionProcessor {
        ...
        original_connect_message: Message) -> Result<bool>
    {
        ...
        let cgw_url = match &self.peer_addr {
            ...
            None => {
                warn!("Unexpected: cannot connect to peer without peer addr set");
                // Return success with no connection rather than failing
                return Ok(false);
            }
        };
    ...
    async fn send_connection_close_event(&mut self) {
        self.peer_addr = None;
